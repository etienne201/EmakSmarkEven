...
export function GuestForm({ onSave, onCancel, initialData, tables, guests, currentAppLang, maxGuestsPerTable = 10 }: GuestFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (currentCount >= maxGuestsPerTable) {
      const errorMsg = currentAppLang === "fr" 
        ? `Cette table est pleine (${maxGuestsPerTable} max).` 
        : `This table is full (${maxGuestsPerTable} max).`;
      ...
      <div className="space-y-4">
        <div>
          ...
          <p className="mt-1.5 text-[10px] text-gray-400 font-medium italic">
            L&apos;invité recevra son invitation et ses instructions dans cette langue.
          </p>
        </div>
        ...
        <p className="mt-1 text-[10px] text-gray-400 font-medium italic">
          L&apos;assignation permet de générer automatiquement le plan de table et d&apos;orienter l&apos;invité dès son arrivée.
        </p>
