...
import { AttendanceModal } from "@frontend/components/AttendanceModal";
...
function GuestContent() {
  ...
  useEffect(() => {
    ...
    if (guestId) {
      // Fetch guest data from API public route (completely bypasses auth guards)
      fetch(`/api/public/guest/${guestId}`)
        .then((res: Response) => res.json())
        .then((resObj: any) => {
          if (resObj.success && resObj.data) {
            const guestData = resObj.data;
            setData(guestData);
            setLoading(false);
            setTimeout(() => setIsPageLoading(false), 300);

            // Load design into Zustand if present
            if (guestData.smartDesign?.templateId) {
              ...
              // Override dynamic fields with guest details
              const customValues = {
              ...
              if (guestData.layoutElements && guestData.layoutElements.length > 0) {
                const customElements = guestData.layoutElements.map((el: any) => {
                  if (el.id === 'g_name') return { ...el, content: customValues.g_name };
                  if (el.id === 'g_table') return { ...el, content: customValues.g_table };
                  if (el.id === 'rsvpQr') return { ...el, content: customValues.rsvpQr };
                  return el;
                });
            ...
            const serverStatus = guestData.checkinStatus === "arrived" ? "Présent" : null;
            ...
            throw new Error(resObj.error || "Failed to parse guest data");
          }
        })
        .catch((err: Error | any) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4">
      ...
      >
        ...
        {/* Dynamic Invitation Smart Canvas or Traditional Header */}
