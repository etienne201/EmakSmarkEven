...
export function PresenceList({ isOpen, onClose, lang, inline = false }: PresenceListProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className={inline ? "w-full h-full flex flex-col" : "fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"}>
          <motion.div
            initial={inline ? { opacity: 0 } : { opacity: 0, scale: 0.9, y: 20 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={inline ? { opacity: 0 } : { opacity: 0, scale: 0.9, y: 20 }}
            className={inline ? "w-full flex-1 flex flex-col h-full" : "w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-gold-light/20 flex flex-col max-h-[85vh]"}
          >
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6 bg-gray-50/50">
              {isLoading ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 text-center">
                  <div className="w-20 h-20 bg-emerald/5 rounded-[2rem] flex items-center justify-center mb-6">
                    <Users className="w-10 h-10 text-emerald/20" />
                  </div>
                  <h3 className="text-lg font-black text-gray-900 mb-2">
                    {searchTerm ? "Aucun résultat trouvé" : "En attente des premiers invités"}
                  </h3>
                  <p className="text-gray-400 text-sm max-w-xs mx-auto font-medium">
                    {searchTerm 
                      ? `Aucune présence ne correspond à "${searchTerm}".` 
                      : "Les noms apparaîtront ici automatiquement dès que les invités scanneront leur QR Code à l'entrée."}
                  </p>
                  {!searchTerm && (
                    <div className="mt-8 px-4 py-2 bg-gray-100 rounded-full text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                      Flux en direct activé
                    </div>
                  )}
                </div>
              ) : (
